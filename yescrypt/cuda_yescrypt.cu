...
#define Bdev(a, b) B[(thread * 16 + (a)) * 16 + (b)]

__launch_bounds__(32, 1)
__global__ void yescrypt_gpu_hash_k0(int threads, uint32_t startNonce)
{
...
__launch_bounds__(32, 1)
__global__ void yescrypt_gpu_hash_k5(int threads, uint32_t startNonce, uint32_t *nonceVector, uint32_t target)
{
	int thread = (blockDim.x * blockIdx.x + threadIdx.x);
	...
	//if (thread < threads)
	{
		...
		in[0] = 0x00000001; in[1] = 0x80000000; in[15] = 0x00002220;
		...
		in[0] = 0x436C6965; in[1] = 0x6E74204B; in[2] = 0x65798000; in[3] = 0x00000000; in[15] = 0x00000250;
		in[4] = in[5] = in[6] = in[7] = in[8] = in[9] = in[10] = in[11] = in[12] = in[13] = in[14] = 0x00000000;
...
__launch_bounds__(32, 1)
__global__ void yescrypt_gpu_hash_k5_hack(int threads, uint32_t startNonce, uint32_t *nonceVector, uint32_t target)
{
	int thread = (blockDim.x * blockIdx.x + threadIdx.x);
	...
	//if (thread < threads)
	{
		...
		in[5] = in[6] = in[7] = in[8] = in[9] = in[10] = in[11] = in[12] = in[13] = in[14] = 0x00000000;
		sha256_round_body(in, state1);	// inner length = 144 * 8 = 1152 = 0x480
...
#define Bdev(a, b) B[(thread * 32 + (a)) * 16 + (b)]

__launch_bounds__(32, 1)
__global__ void yescrypt_gpu_hash_k0_r16(int threads, uint32_t startNonce)
{
	int thread = (blockDim.x * blockIdx.x + threadIdx.x);
...
__launch_bounds__(32, 1)
__global__ void yescrypt_gpu_hash_k5_r16(int threads, uint32_t startNonce, uint32_t *nonceVector, uint32_t target)
{
	int thread = (blockDim.x * blockIdx.x + threadIdx.x);
	...
	//if (thread < threads)
	{
		...
		for (int i = 0; i < 32; i++)
		{
			...
			in[8] = cuda_swab32(in[8]); in[9] = cuda_swab32(in[9]); in[10] = cuda_swab32(in[10]); in[11] = cuda_swab32(in[11]);
			in[12] = cuda_swab32(in[12]); in[13] = cuda_swab32(in[13]); in[14] = cuda_swab32(in[14]); in[15] = cuda_swab32(in[15]);
			sha256_round_body(in, state1);	// inner 2112byte
		}

		in[0] = 0x00000001; in[1] = 0x80000000; in[15] = 0x00004220;
		in[2] = in[3] = in[4] = in[5] = in[6] = in[7] = in[8] = in[9] = in[10] = in[11] = in[12] = in[13] = in[14] = 0x00000000;
		sha256_round_body(in, state1);	// inner length = 2116 * 8 = 16928 = 0x4220
...
#define Bdev(a, b) B[(thread * 128 + (a)) * 16 + (b)]

__launch_bounds__(32, 1)
__global__ void yescrypt_gpu_hash_k0_r16v2(int threads, uint32_t startNonce)
{
	int thread = (blockDim.x * blockIdx.x + threadIdx.x);
...
__launch_bounds__(32, 1)
__global__ void yescrypt_gpu_hash_k5_r16v2(int threads, uint32_t startNonce, uint32_t *nonceVector, uint32_t target)
{
	int thread = (blockDim.x * blockIdx.x + threadIdx.x);
	...
	//if (thread < threads)
	{
		...
		for (int i = 0; i < 128; i++)
		{
			...
			in[8] = cuda_swab32(in[8]); in[9] = cuda_swab32(in[9]); in[10] = cuda_swab32(in[10]); in[11] = cuda_swab32(in[11]);
			in[12] = cuda_swab32(in[12]); in[13] = cuda_swab32(in[13]); in[14] = cuda_swab32(in[14]); in[15] = cuda_swab32(in[15]);
			sha256_round_body(in, state1);	// inner 8256byte
		}

		in[0] = 0x00000001; in[1] = 0x80000000; in[15] = 0x00010220;
		in[2] = in[3] = in[4] = in[5] = in[6] = in[7] = in[8] = in[9] = in[10] = in[11] = in[12] = in[13] = in[14] = 0x00000000;
		sha256_round_body(in, state1);	// inner length = 8260 * 8 = 66080 = 0x10220
		...
		in[0] = 0x50505450; in[1] = 0x5075624B; in[2] = 0x65798000; in[3] = 0x00000000; in[15] = 0x00000250;
		in[4] = in[5] = in[6] = in[7] = in[8] = in[9] = in[10] = in[11] = in[12] = in[13] = in[14] = 0x00000000;
		sha256_round_body(in, state1);	// inner length = 74 * 8 = 592 = 0x250
...
#define Bdev(a, b) B[(thread * 64 + (a)) * 16 + (b)]

__launch_bounds__(32, 1)
__global__ void yescrypt_gpu_hash_k0_r32(int threads, uint32_t startNonce)
{
	int thread = (blockDim.x * blockIdx.x + threadIdx.x);
...
__launch_bounds__(32, 1)
__global__ void yescrypt_gpu_hash_k5_r32(int threads, uint32_t startNonce, uint32_t *nonceVector, uint32_t target)
{
	int thread = (blockDim.x * blockIdx.x + threadIdx.x);
	...
	//if (thread < threads)
	{
		...
		for (int i = 0; i < 64; i++)
		{
			...
			in[8] = cuda_swab32(in[8]); in[9] = cuda_swab32(in[9]); in[10] = cuda_swab32(in[10]); in[11] = cuda_swab32(in[11]);
			in[12] = cuda_swab32(in[12]); in[13] = cuda_swab32(in[13]); in[14] = cuda_swab32(in[14]); in[15] = cuda_swab32(in[15]);
			sha256_round_body(in, state1);	// inner 4160byte
		}

		in[0] = 0x00000001; in[1] = 0x80000000; in[15] = 0x00008220;
		in[2] = in[3] = in[4] = in[5] = in[6] = in[7] = in[8] = in[9] = in[10] = in[11] = in[12] = in[13] = in[14] = 0x00000000;
		sha256_round_body(in, state1);	// inner length = 4164 * 8 = 33312 = 0x8220
		...
		in[0] = 0x57617669; in[1] = 0x42616E61; in[2] = 0x6E618000; in[3] = 0x00000000; in[15] = 0x00000250;
		in[4] = in[5] = in[6] = in[7] = in[8] = in[9] = in[10] = in[11] = in[12] = in[13] = in[14] = 0x00000000;
		sha256_round_body(in, state1);	// inner length = 74 * 8 = 592 = 0x250
...
#define Bdev(a, b) B[(thread * 16 + (a)) * 16 + (b) * 4 + threadIdx.x]
...
#define Bdev(a) B[(thread * 16 + (a)) * 16 + threadIdx.x]
#define Sdev(a) S[(thread_part_4 * 128 + (a)) * 16 + threadIdx.x]
#define Shared(a) *(uint2*)&shared_mem[(threadIdx.y * 512 + (a)) * 4 + (threadIdx.x & 2)]

__global__ __launch_bounds__(32, 1) void yescrypt_gpu_hash_k2c(int threads, uint32_t startNonce, uint32_t offset1, uint32_t offset2, uint32_t count, uint32_t index)
{
	...
	uint32_t *v = &V[blockIdx.x * 2048 * 16 * 32 + threadIdx.y * 16 + threadIdx.x];
...
#pragma unroll
		...
		for (n = p2floor(count*index), i = count*index; i < count*(index + 1); i++) {
#pragma unroll
			for (k = 0; k < 16; k++)
				__stL1(&Vdev(i, k), x[k]);
			...
			if (i > 1) {
				if ((i & (i - 1)) == 0) n = i;
				j = __shfl_sync(0xFFFFFFFF, x[15], 0, 16) & (n - 1);
				j += i - n;
...
#pragma unroll
			...
			x0 = __shfl_sync(0xFFFFFFFF, x[15], 0 + (threadIdx.x & 3), 16);
			x1 = __shfl_sync(0xFFFFFFFF, x[15], 4 + (threadIdx.x & 3), 16);
			x2 = __shfl_sync(0xFFFFFFFF, x[15], 8 + (threadIdx.x & 3), 16);
			x3 = __shfl_sync(0xFFFFFFFF, x[15], 12 + (threadIdx.x & 3), 16);
			SALSA_CORE(x0, x1, x2, x3);
			if (threadIdx.x < 4) x[15] = x0;
			else if (threadIdx.x < 8) x[15] = x1;
			else if (threadIdx.x < 12) x[15] = x2;
			else x[15] = x3;
...
__global__ __launch_bounds__(32, 1) void yescrypt_gpu_hash_k2c1(int threads, uint32_t startNonce, uint32_t offset1, uint32_t offset2)
{
	uint32_t thread_part_16 = (2 * blockIdx.x + threadIdx.y);
	uint32_t thread_part_4 = thread_part_16 + offset1;
	uint32_t thread = thread_part_16 + offset2;
...
#pragma unroll
		...
		for (uint32_t z = 0; z < 684; z++)
		{
			j = __shfl_sync(0xFFFFFFFF, x[15], 0, 16) & 2047;
...
#define Bdev(a, b) B[(thread * 32 + (a)) * 16 + (b) * 4 + threadIdx.x]
#define Sdev(a, b) S[(thread_part_4 * 128 + (a)) * 16 + (b) * 4 + threadIdx.x]

__global__ __launch_bounds__(32, 1)
void yescrypt_gpu_hash_k1_r16(int threads, uint32_t startNonce, uint32_t offset)
{
	uint32_t thread_part_4 = (8 * blockIdx.x + threadIdx.y);
	uint32_t thread = thread_part_4 + offset;
...
#define Vdev(a, b) V[((thread_part_16 * N + (a)) * r * 2 + (b)) * 16 + threadIdx.x]
#define Bdev(a) B[(thread * 32 + (a)) * 16 + threadIdx.x]
#define Sdev(a) S[(thread_part_4 * 128 + (a)) * 16 + threadIdx.x]

__global__ __launch_bounds__(32, 1) void yescrypt_gpu_hash_k2c_r16(int threads, uint32_t startNonce, uint32_t offset1, uint32_t offset2, uint32_t count, uint32_t index)
{
	uint32_t thread_part_16 = (2 * blockIdx.x + threadIdx.y);
	uint32_t thread_part_4 = thread_part_16 + offset1;
	uint32_t thread = thread_part_16 + offset2;
...
__global__ __launch_bounds__(32, 1) void yescrypt_gpu_hash_k2c1_r16(int threads, uint32_t startNonce, uint32_t offset1, uint32_t offset2)
{
	uint32_t thread_part_16 = (2 * blockIdx.x + threadIdx.y);
	uint32_t thread_part_4 = thread_part_16 + offset1;
	uint32_t thread = thread_part_16 + offset2;
...
#define Bdev(a, b) B[(thread * 128 + (a)) * 16 + (b) * 4 + threadIdx.x]
#define Sdev(a, b) S[(thread_part_4 * 128 + (a)) * 16 + (b) * 4 + threadIdx.x]

__global__ __launch_bounds__(32, 1)
void yescrypt_gpu_hash_k1_r16v2(int threads, uint32_t startNonce, uint32_t offset, uint32_t offset_b)
{
	uint32_t thread_part_4 = (8 * blockIdx.x + threadIdx.y);
	uint32_t thread = thread_part_4 + offset;
	...
	//if (thread < threads)
	{
		...
		x[0] = Bdev(offset_b * 32 + 0, 0);
		x[1] = Bdev(offset_b * 32 + 0, 1);
		x[2] = Bdev(offset_b * 32 + 0, 2);
		x[3] = Bdev(offset_b * 32 + 0, 3);
		x[4] = Bdev(offset_b * 32 + 1, 0);
		x[5] = Bdev(offset_b * 32 + 1, 1);
		x[6] = Bdev(offset_b * 32 + 1, 2);
		x[7] = Bdev(offset_b * 32 + 1, 3);
		...
		Bdev(offset_b * 32 + 0, 0) = x[0];
		Bdev(offset_b * 32 + 0, 1) = x[1];
		Bdev(offset_b * 32 + 0, 2) = x[2];
		Bdev(offset_b * 32 + 0, 3) = x[3];
		Bdev(offset_b * 32 + 1, 0) = x[4];
		Bdev(offset_b * 32 + 1, 1) = x[5];
		Bdev(offset_b * 32 + 1, 2) = x[6];
		Bdev(offset_b * 32 + 1, 3) = x[7];
	}
}
...
#define Vdev(a, b) V[((thread_part_16 * N + (a)) * r * 2 + (b)) * 16 + threadIdx.x]
#define Bdev(a) B[(thread * 128 + (a)) * 16 + threadIdx.x]
#define Sdev(a) S[(thread_part_4 * 128 + (a)) * 16 + threadIdx.x]

__global__ __launch_bounds__(32, 1) void yescrypt_gpu_hash_k2c_r16v2(int threads, uint32_t startNonce, uint32_t offset1, uint32_t offset2, uint32_t count, uint32_t index, uint32_t offset_b)
{
	uint32_t thread_part_16 = (2 * blockIdx.x + threadIdx.y);
	uint32_t thread_part_4 = thread_part_16 + offset1;
	uint32_t thread = thread_part_16 + offset2;
...
#pragma unroll
		...
		for (k = 0; k < r * 2; k++)
			Bdev(offset_b * 32 + k) = x[k];
	}
}

__global__ __launch_bounds__(32, 1) void yescrypt_gpu_hash_k2c1_r16v2(int threads, uint32_t startNonce, uint32_t offset1, uint32_t offset2, uint32_t offset_b)
{
	uint32_t thread_part_16 = (2 * blockIdx.x + threadIdx.y);
	uint32_t thread_part_4 = thread_part_16 + offset1;
	uint32_t thread = thread_part_16 + offset2;
...
#define Bdev(a, b) B[(thread * 64 + (a)) * 16 + (b) * 4 + threadIdx.x]
#define Sdev(a, b) S[(thread_part_4 * 128 + (a)) * 16 + (b) * 4 + threadIdx.x]

__global__ __launch_bounds__(32, 1)
void yescrypt_gpu_hash_k1_r32(int threads, uint32_t startNonce, uint32_t offset)
{
	uint32_t thread_part_4 = (8 * blockIdx.x + threadIdx.y);
	uint32_t thread = thread_part_4 + offset;
...
#define Bdev(a) B[(thread * r*2 + (a)) * 16 + threadIdx.x]
#define Sdev(a) S[(thread_part_4 * 128 + (a)) * 16 + threadIdx.x]

__global__ __launch_bounds__(32, 1) void yescrypt_gpu_hash_k2c_r32(int threads, uint32_t startNonce, uint32_t offset1, uint32_t offset2, uint32_t count, uint32_t index)
{
	uint32_t thread_part_16 = (2 * blockIdx.x + threadIdx.y);
	uint32_t thread_part_4 = thread_part_16 + offset1;
	uint32_t thread = thread_part_16 + offset2;
...
__global__ __launch_bounds__(32, 1) void yescrypt_gpu_hash_k2c1_r32(int threads, uint32_t startNonce, uint32_t offset1, uint32_t offset2)
{
	uint32_t thread_part_16 = (2 * blockIdx.x + threadIdx.y);
	uint32_t thread_part_4 = thread_part_16 + offset1;
	uint32_t thread = thread_part_16 + offset2;
...
__host__
void yescrypt_setTarget(uint32_t pdata[20])
{
	uint32_t h[8], data[16];
...
__host__ void yescrypt_cpu_hash_32(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *resultnonces, uint32_t target)
{
	...
	uint32_t loop_count;
	if (cuda_arch[dev_id] > 500) loop_count = 1;
	else if (cuda_arch[dev_id] == 500) loop_count = 2;
	else loop_count = 4;

	yescrypt_gpu_hash_k0 << <grid, block >> >(threads, startNounce);
	for (uint32_t i = 0; i < 4; i++)
	{
		yescrypt_gpu_hash_k1 << <grid, block2 >> > (threads, startNounce, i * (threads >> 2));
		for (uint32_t j = 0; j < 4; j++)
		{
			for (uint32_t k = 0; k < loop_count; k++)
				yescrypt_gpu_hash_k2c << <grid, block3, 16384 >> > (threads, startNounce, j * (threads >> 4), (i * 4 + j) * (threads >> 4), 2048 / loop_count, k);
			yescrypt_gpu_hash_k2c1 << <grid, block3, 16384 >> > (threads, startNounce, j * (threads >> 4), (i * 4 + j) * (threads >> 4));
		}
	}
	yescrypt_gpu_hash_k5_hack << <grid, block >> > (threads, startNounce, d_GNonce[thr_id], target);
...
__host__ void yescryptr8_cpu_hash_32(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *resultnonces, uint32_t target)
{
	int dev_id = device_map[thr_id % MAX_GPUS];
	CUDA_SAFE_CALL(cudaMemset(d_GNonce[thr_id], 0, 2 * sizeof(uint32_t)));
	...
	yescrypt_gpu_hash_k5 << <grid, block >> > (threads, startNounce, d_GNonce[thr_id], target);
	CUDA_SAFE_CALL(cudaGetLastError());
	if(opt_debug)
		CUDA_SAFE_CALL(cudaDeviceSynchronize());
...
__host__ void yescryptr16_cpu_hash_32(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *resultnonces, uint32_t target)
{
	int dev_id = device_map[thr_id % MAX_GPUS];
	CUDA_SAFE_CALL(cudaMemset(d_GNonce[thr_id], 0, 2 * sizeof(uint32_t)));
	...
	if (cuda_arch[dev_id] < 500) {
		cudaFuncSetCacheConfig(yescrypt_gpu_hash_k2c_r16, cudaFuncCachePreferShared);
		cudaFuncSetCacheConfig(yescrypt_gpu_hash_k2c1_r16, cudaFuncCachePreferShared);
	}
	else if (cuda_arch[dev_id] >= 700) {
		cudaFuncSetAttribute(yescrypt_gpu_hash_k2c_r16, cudaFuncAttributePreferredSharedMemoryCarveout, 100);
		cudaFuncSetAttribute(yescrypt_gpu_hash_k2c1_r16, cudaFuncAttributePreferredSharedMemoryCarveout, 100);
	}

	uint32_t loop_count;
	if (cuda_arch[dev_id] > 500) loop_count = 4;
	else if (cuda_arch[dev_id] == 500) loop_count = 8;
	else loop_count = 16;

	yescrypt_gpu_hash_k0_r16 << <grid, block >> >(threads, startNounce);
	for (uint32_t i = 0; i < 4; i++)
	{
		yescrypt_gpu_hash_k1_r16 << <grid, block2 >> > (threads, startNounce, i * (threads >> 2));
		for (uint32_t j = 0; j < 4; j++)
		{
			for (uint32_t k = 0; k < loop_count; k++)
				yescrypt_gpu_hash_k2c_r16 << <grid, block3, 16384 >> > (threads, startNounce, j * (threads >> 4), (i * 4 + j) * (threads >> 4), 4096 / loop_count, k);
			yescrypt_gpu_hash_k2c1_r16 << <grid, block3, 16384 >> > (threads, startNounce, j * (threads >> 4), (i * 4 + j) * (threads >> 4));
		}
	}
	yescrypt_gpu_hash_k5_r16 << <grid, block >> > (threads, startNounce, d_GNonce[thr_id], target);
	CUDA_SAFE_CALL(cudaGetLastError());
	if(opt_debug)
		CUDA_SAFE_CALL(cudaDeviceSynchronize());
...
__host__ void yescryptr16v2_cpu_hash_32(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *resultnonces, uint32_t target)
{
	int dev_id = device_map[thr_id % MAX_GPUS];
	CUDA_SAFE_CALL(cudaMemset(d_GNonce[thr_id], 0, 2 * sizeof(uint32_t)));
	...
	if (cuda_arch[dev_id] < 500) {
		cudaFuncSetCacheConfig(yescrypt_gpu_hash_k2c_r16v2, cudaFuncCachePreferShared);
		cudaFuncSetCacheConfig(yescrypt_gpu_hash_k2c1_r16v2, cudaFuncCachePreferShared);
	}
	else if (cuda_arch[dev_id] >= 700) {
		cudaFuncSetAttribute(yescrypt_gpu_hash_k2c_r16v2, cudaFuncAttributePreferredSharedMemoryCarveout, 100);
		cudaFuncSetAttribute(yescrypt_gpu_hash_k2c1_r16v2, cudaFuncAttributePreferredSharedMemoryCarveout, 100);
	}
	...
	yescrypt_gpu_hash_k0_r16v2 << <grid, block >> >(threads, startNounce);
	for (uint32_t l = 0; l < 4; l++)
	{
		for (uint32_t i = 0; i < 4; i++)
		{
			yescrypt_gpu_hash_k1_r16v2<< <grid, block2 >> > (threads, startNounce, i * (threads >> 2), l);
			for (uint32_t j = 0; j < 4; j++)
			{
				for (uint32_t k = 0; k < loop_count; k++)
					yescrypt_gpu_hash_k2c_r16v2 << <grid, block3, 16384 >> > (threads, startNounce, j * (threads >> 4), (i * 4 + j) * (threads >> 4), 4096 / loop_count, k, l);
				yescrypt_gpu_hash_k2c1_r16v2 << <grid, block3, 16384 >> > (threads, startNounce, j * (threads >> 4), (i * 4 + j) * (threads >> 4), l);
			}
		}
	}
	yescrypt_gpu_hash_k5_r16v2 << <grid, block >> > (threads, startNounce, d_GNonce[thr_id], target);
	CUDA_SAFE_CALL(cudaGetLastError());
	if (opt_debug)
		CUDA_SAFE_CALL(cudaDeviceSynchronize());
...
__host__ void yescryptr32_cpu_hash_32(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *resultnonces, uint32_t target)
{
	int dev_id = device_map[thr_id % MAX_GPUS];
	CUDA_SAFE_CALL(cudaMemset(d_GNonce[thr_id], 0, 2 * sizeof(uint32_t)));
	...
	if (cuda_arch[dev_id] < 500) {
		cudaFuncSetCacheConfig(yescrypt_gpu_hash_k2c_r32, cudaFuncCachePreferShared);
		cudaFuncSetCacheConfig(yescrypt_gpu_hash_k2c1_r32, cudaFuncCachePreferShared);
	}
	else if (cuda_arch[dev_id] >= 700) {
		cudaFuncSetAttribute(yescrypt_gpu_hash_k2c_r32, cudaFuncAttributePreferredSharedMemoryCarveout, 100);
		cudaFuncSetAttribute(yescrypt_gpu_hash_k2c1_r32, cudaFuncAttributePreferredSharedMemoryCarveout, 100);
	}

	uint32_t loop_count;
	if (cuda_arch[dev_id] > 500) loop_count = 8;
	else if (cuda_arch[dev_id] == 500) loop_count = 16;
	else loop_count = 32;

	yescrypt_gpu_hash_k0_r32 << <grid, block >> >(threads, startNounce);
	for (uint32_t i = 0; i < 4; i++)
	{
		yescrypt_gpu_hash_k1_r32 << <grid, block2 >> > (threads, startNounce, i * (threads >> 2));
		for (uint32_t j = 0; j < 4; j++)
		{
			for (uint32_t k = 0; k < loop_count; k++)
				yescrypt_gpu_hash_k2c_r32 << <grid, block3, 16384 >> > (threads, startNounce, j * (threads >> 4), (i * 4 + j) * (threads >> 4), 4096 / loop_count, k);
			yescrypt_gpu_hash_k2c1_r32 << <grid, block3, 16384 >> > (threads, startNounce, j * (threads >> 4), (i * 4 + j) * (threads >> 4));
		}
	}
	yescrypt_gpu_hash_k5_r32 << <grid, block >> > (threads, startNounce, d_GNonce[thr_id], target);
	CUDA_SAFE_CALL(cudaGetLastError());
	if (opt_debug)
		CUDA_SAFE_CALL(cudaDeviceSynchronize());
